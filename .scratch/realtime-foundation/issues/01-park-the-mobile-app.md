...
- [ ] `apps/mobile/package.json` no longer defines `lint`, `typecheck` or `test`; `start` stays so `pnpm dev:mobile` works
- [ ] The root `typecheck:mobile` script in `package.json` is removed
- [ ] `AGENTS.md` (Repo Shape, the `apps/mobile` line) and `CONTRIBUTING.md` (the workspace table) say the app is parked pending a rewrite and not covered by the gates
- [ ] Nothing else under `apps/mobile` changes
- [ ] `pnpm lint`, `pnpm typecheck` and `pnpm test:run` run nothing in `apps/mobile` (check turbo's task list in the output)
