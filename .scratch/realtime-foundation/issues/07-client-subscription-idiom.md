...
- Implemented on `claude/realtime-foundation-tickets-8ercvt` together with 08.
  ...
  - Tests: `packages/shared-react/__tests__/realtime/use-realtime-subscription.test.tsx` (real `useSubscription`, recorded `subscribe`); the deleted web tests' cache assertions live on in `packages/shared-react/__tests__/hooks/{recipes,ratings}-subscription.test.tsx`; the remaining web hook tests emit tracked envelopes through `apps/web/__tests__/hooks/realtime-test-utils.ts` (`trackedEvent`, `adaptLegacyFrame`).
